$(document).ready(function () {
    var id, n, host, getHost, ossPath, suffix, speed, currentN, toN;
    init();

    function init() {
        id = getQueryString('id');
        n = parseInt(id.split('-')[1]);
        if (typeof n == "undefined" || isNaN(n) || n < 1) {
            return false;
        }

        host = 'https://dzprivate.eyexpo.net.cn/static/upload/oss/3d';
        getHost = getQueryString('host');
        if (typeof getHost != "undefined" && getHost != '' && getHost != null) {
            host = getHost;
        }
        ossPath = host + '/' + id + '/';
        suffix = '.jpg';
        ...
        $('#img img').attr('src', ossPath + currentN + suffix)
    }
